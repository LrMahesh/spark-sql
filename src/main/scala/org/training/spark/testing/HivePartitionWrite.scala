...
object HivePartitionWrite extends App {
  ...
  val conf = new SparkConf().setMaster("local").setAppName("Hive partition write")
  ...
  val salesDF = hc.read.format("com.databricks.spark.csv").options(loadOptions).load("/home/hduser/Projects/spark-sql/src/main/resources/sales.csv")
